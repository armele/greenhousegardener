...
public class GreenhouseGardenerMod
{
    ...
    public void onAddReloadListener(final AddReloadListenerEvent event)
    {
        ...
        event.addListener(GreenhouseCropProductListener.INSTANCE);
    }

    @SubscribeEvent
    public void onDatapackSync(final OnDatapackSyncEvent event)
    {
        final SyncGreenhouseClimateItemsMessage message =
            new SyncGreenhouseClimateItemsMessage(GreenhouseClimateItemValueListener.INSTANCE.snapshotValues());
        event.getRelevantPlayers().forEach(message::sendToPlayer);
    }
